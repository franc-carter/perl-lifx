...
my @setcolor = (
    0x31,0x00,0x00,0x34,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
    0x4c,0x49,0x46,0x58,0x56,0x32,0x00,0x00,0x00,0x03,0x1d,0x97,0x71,0xe8,0x86,0x13,
    0x66,0x00,0x00,0x00,0x00,0xe3,0x18,0xb7,0x5e,0x65,0xa6,0xac,0x0d,0xe8,0x03,0x00,
    0x00
);

my $setcolor = pack('C*', @setcolor);

printPacket($setcolor);

my $gw = inet_aton("192.168.2.8");
my $to = sockaddr_in($port, $gw);
$socket->send($setcolor, 0, $to);
